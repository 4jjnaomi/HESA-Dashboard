...
def create_sidebar():
    ...
    search_bar = dbc.Input(id="search_input", type="search", placeholder="Search for a HEI", className="mb-3", style={"padding": 10})
    collapse = dbc.Collapse(children=[search_bar, dbc.Nav(create_nav_links(universities), vertical=True, id="sidebar-nav")], id="collapse")
    toggle_button = dbc.Button("Choose a HEI", id="toggle", className="mb-3", color="primary")
...
class_dropdown = dbc.Select(id="class-dropdown", options=[{"label": cls, "value": cls} for cls in ["Building and spaces", "Energy", "Emissions and waste", "Transport and environment", "Finances and people"]], placeholder="Choose a class to see options for 'Category Marker'")

category_marker_dropdown = dbc.Select(id="category-marker-dropdown", options=[], placeholder="Choose a category marker to see a graph")
...
def layout(he_provider=None):
    ...
    row_two = dbc.Row([dbc.Col(children=[html.P(f"Use this page to see how {decoded_he_provider} has performed between 2018/19 - 2012/22 in various environmental categories."), html.P("You can analyse other universities using the button to the side.", style={"font-weight": "bold"})], width=12)])
    row_three = dbc.Row([dbc.Col([html.P(children=["Class", class_dropdown], style={"font-size": 20})], width=6), dbc.Col([html.P(children=["Category Marker", category_marker_dropdown], style={"font-size": 20})], width=6)])
    row_four = dbc.Row([dbc.Col(children=[dcc.Graph(figure=line_chart, id='overview_line_chart')], width=12)])
    page_layout = dbc.Container([dbc.Row([dbc.Col(create_sidebar(), width=2), dbc.Col([row_one, row_two, row_three, row_four], width=10)])])
    non_existent_uni_layout = dbc.Container([dbc.Row([dbc.Col(create_sidebar(), width=2), dbc.Col([html.H1("University not found"), html.P("The university you are looking for does not exist in our database."), html.P("Please edit the url to choose a different university or click on a university from the sidebar.")], width=10)])])
...
def update_nav(search_value):
    universities = generate_sidebar_links()
    links = universities if not search_value else [uni for uni in universities if search_value.lower() in uni["children"].lower()]
